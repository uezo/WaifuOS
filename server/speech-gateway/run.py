...
# NIJIVOICE
SPGW_NIJIVOICE_ENABLED = os.getenv("SPGW_NIJIVOICE_ENABLED", "false").lower() in ("true", "1", "yes")
SPGW_NIJIVOICE_API_KEY = os.getenv("SPGW_NIJIVOICE_API_KEY")
SPGW_NIJIVOICE_LANGUAGES = os.getenv("SPGW_NIJIVOICE_LANGUAGES")
SPGW_NIJIVOICE_DEFAULT_SPEAKER = os.getenv("SPGW_NIJIVOICE_DEFAULT_SPEAKER")
# Defaults
...
async def lifespan(app: FastAPI):
    ...
    if SPGW_AZURE_ENABLED and 'azure_gateway' in globals():
        await azure_gateway.shutdown()
    if SPGW_OPENAI_ENABLED and 'openai_gateway' in globals():
        await openai_gateway.shutdown()
    if SPGW_VOICEVOX_ENABLED and 'voicevox_gateway' in globals():
        await voicevox_gateway.shutdown()
    if SPGW_SBV2_ENABLED and 'sbv2_gateway' in globals():
        await sbv2_gateway.shutdown()
    if SPGW_NIJIVOICE_ENABLED and 'nijivoice_gateway' in globals():
        await nijivoice_gateway.shutdown()

# Create API app
app = FastAPI(
    title="Speech Gateway",
    openapi_url="/speech-gateway/openapi.json",
    servers=[{"url": "/speech-gateway"}],
    lifespan=lifespan,
)

# Unified gateway
unified_gateway = UnifiedGateway(debug=True)
app.include_router(unified_gateway.get_router(), tags=["Gateways"])

# Create service gateways
if SPGW_AZURE_ENABLED:
    azure_gateway = AzureGateway(api_key=SPGW_AZURE_API_KEY, cache_dir="cache/azure", performance_recorder=performance_recorder, region=SPGW_AZURE_REGION, debug=SPGW_DEBUG)
    ...
    app.include_router(azure_gateway.get_router(), prefix="/azure", tags=["Gateways"])
    logger.info("[Gateway] Azure on /azure")

if SPGW_OPENAI_ENABLED:
    openai_gateway = OpenAIGateway(api_key=SPGW_OPENAI_API_KEY, cache_dir="cache/openai", performance_recorder=performance_recorder, debug=SPGW_DEBUG)
    unified_gateway.add_gateway(
        service_name="openai",
    ...
    app.include_router(openai_gateway.get_router(), prefix="/openai", tags=["Gateways"])
    logger.info(f"[Gateway] OpenAI on /openai")

if SPGW_VOICEVOX_ENABLED:
    voicevox_gateway = VoicevoxGateway(base_url=SPGW_VOICEVOX_URL, cache_dir="cache/voicevox", performance_recorder=performance_recorder, debug=SPGW_DEBUG)
    unified_gateway.add_gateway(
    ...
    app.include_router(voicevox_gateway.get_router(), prefix="/voicevox", tags=["Gateways"])
    logger.info(f"[Gateway] VOICEVOX on /voicevox")

if SPGW_AIVISSPEECH_ENABLED:
    aivisspeech_gateway = VoicevoxGateway(base_url=SPGW_AIVISSPEECH_URL, cache_dir="cache/aivis-speech", performance_recorder=performance_recorder, debug=SPGW_DEBUG)
    unified_gateway.add_gateway(
    ...
    app.include_router(aivisspeech_gateway.get_router(), prefix="/aivis-speech", tags=["Gateways"])
    logger.info(f"[Gateway] AivisSpeech on /aivis-speech")

if SPGW_SBV2_ENABLED:
    sbv2_gateway = StyleBertVits2Gateway(base_url=SPGW_SBV2_URL, cache_dir="cache/sbv2", performance_recorder=performance_recorder, debug=SPGW_DEBUG)
    unified_gateway.add_gateway(
        service_name="sbv2",
    ...
    app.include_router(sbv2_gateway.get_router(), prefix="/sbv2", tags=["Gateways"])
    logger.info(f"[Gateway] Style-Bert-VITS2 on /sbv2")

if SPGW_NIJIVOICE_ENABLED:
    nijivoice_gateway = NijiVoiceEncodedGateway(api_key=SPGW_NIJIVOICE_API_KEY, cache_dir="cache/nijivoice", performance_recorder=performance_recorder, debug=SPGW_DEBUG)
    unified_gateway.add_gateway(
        service_name="nijivoice",
        gateway=nijivoice_gateway,
        languages=SPGW_NIJIVOICE_LANGUAGES.split(",") if SPGW_NIJIVOICE_LANGUAGES else None,
        default_speaker=SPGW_NIJIVOICE_DEFAULT_SPEAKER,
        default=SPGW_DEFAULT_SERVICE == "nijivoice"
    )
    app.include_router(nijivoice_gateway.get_router(), prefix="/nijivoice", tags=["Gateways"])
